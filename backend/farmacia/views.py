...
class MovimentacaoViewSet(viewsets.ModelViewSet):
    ...
    def relatorio_geral(self, request):
        ...
        if data_inicio_str and not data_inicio:
            return Response({"erro": "data_inicio inválida. Use YYYY-MM-DD."}, status=400)

        if data_fim_str and not data_fim:
            return Response({"erro": "data_fim inválida. Use YYYY-MM-DD."}, status=400)

        if data_inicio and data_fim and data_inicio > data_fim:
            return Response({"erro": "data_inicio não pode ser maior que data_fim."}, status=400)
        ...
        total_entradas = qs.filter(tipo="E").aggregate(total=Sum("quantidade"))["total"] or 0
        ...
        return Response({
            "filtros": {"data_inicio": data_inicio_str, "data_fim": data_fim_str},
...
def cadastrar_usuario(request):
    ...
    if not usuario or len(usuario) < 4:
        return Response({"erro": "Usuário deve ter no mínimo 4 caracteres."}, status=status.HTTP_400_BAD_REQUEST)
    if not senha or len(senha) < 6:
        return Response({"erro": "Senha deve ter no mínimo 6 caracteres."}, status=status.HTTP_400_BAD_REQUEST)
...
def login_usuario(request):
    login  = (request.data.get("usuario") or request.data.get("login") or "").strip()
    senha  = request.data.get("senha") or ""
    ...
    if not user.is_active:
        return Response({"erro": "Conta desativada."}, status=status.HTTP_401_UNAUTHORIZED)
      ...
      <table width="560" cellpadding="0" cellspacing="0"
        ...
        <tr>
          <td style="padding:40px;">
            <p style="margin:0 0 16px;font-size:15px;color:#334155;">Olá, <strong>{nome_usuario}</strong>!</p>
        ...
        <tr>
          <td style="background:#f8faff;padding:20px 40px;text-align:center;border-top:1px solid #e2e8f0;">
            <p style="margin:0;font-size:12px;color:#94a3b8;">© GestMed — Sistema de Gestão Farmacêutica</p>
          </td>
        </tr>
      </table>
    </td></tr>
...
def solicitar_recuperacao_senha(request):
    ...
    resposta_padrao = Response(
        {"sucesso": True, "mensagem": "Se o email existir, o link de recuperação será enviado."},
    ...
    site_url   = getattr(settings, "SITE_URL", "http://127.0.0.1:5500/Frontend")
    reset_link = f"{site_url}/html/reset-password.html?uid={uid}&token={token}"
    ...
    send_mail(
        ...
        html_message=_html_email_recuperacao(nome_usuario, reset_link),
        fail_silently=False,
    )
...
def redefinir_senha(request):
    ...
    return Response({"sucesso": True, "mensagem": "Senha redefinida com sucesso."})